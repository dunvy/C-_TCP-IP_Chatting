...
void SendMsg(SOCKET hClntSock, char *msg, int len)
{
    ...
    if(!strncmp(msg, "/f" , 2) || !strncmp(msg, "/F", 2))
    {
        std::cout << "파일전송 해야되는디 여그 오냐?" << std::endl;
        // 파일이름
        std::string sendFile;
        ...
        long filesize;
        int sendbyte;
        char fileMsg[BUF_SIZE];

        char fileName[BUF_SIZE];        
        strcpy(fileName, sendFile.c_str());

        fp = fopen(fileName, "rt"); // 파일열기

        fseek(fp,0,SEEK_END);
        filesize = ftell(fp);
        fseek(fp,0,SEEK_SET);

        while((sendbyte = fread(fileMsg,sizeof(char),sizeof(fileMsg), fp)) > 0)
        {
            for(int i = 0; i < singleChat.size(); i++)
            {
                if(i%2 == 0)
                {
                    ...
                    {
                        send(singleChat[i+1], fileMsg, len, 0);
                    }
                }
                else
                {
